using UnityEngine;
using UnityEngine.UI;

[RequireComponent(typeof(AudioSource))]
public class BarVisulization : MonoBehaviour
{
    public AudioClip[] clips;
    public Image[] barsSprites;
    public Slider musicSlider;
    [Range(0, 10)] public float colorMultiplyer = 1;
    [Range(0, 1)] public float s = 1;
    [Range(0, 1)] public float v = 1;
    private int index = 0;
    private float musicLength;
    private AudioSource audioSource;

    void Start()
    {
        audioSource = GetComponent<AudioSource>();
        ChangeSound();
    }

    void Update()
    {
        Visulization();

        if (Input.GetMouseButtonDown(0))
        {
            ChangeSound();
        }

        MusicSlider();
    }

    void Visulization()
    {
        var musicData = new float[barsSprites.Length];
        audioSource.GetSpectrumData(musicData, 1, FFTWindow.Blackman);

        var i = 0;
        while (i < barsSprites.Length)
        {
            barsSprites[i].transform.localScale = new Vector3(0.1f, musicData[i] * 2, 1);
            barsSprites[i].color = HSVtoRGB(musicData[i] * colorMultiplyer, s, v, 1);

            i++;
        }
    }

    void MusicSlider()
    {
        musicLength = audioSource.time;
        musicSlider.value = musicLength / audioSource.clip.length;
    }

    void ChangeSound()
    {
        index++;
        if (index > clips.Length - 1)
        {
            index = 0;
        }
        audioSource.clip = clips[index];

        audioSource.Play();
    }


    #region Static

    public static Color HSVtoRGB(float hue, float saturation, float value, float alpha)
    {
        saturation = Mathf.Clamp(saturation, 0, 1f);

        value = Mathf.Clamp(value, 0, 1f);

        hue = Mathf.Clamp(hue, 0.001f, 0.999f);

        saturation = Mathf.Min(saturation, 0.999f);

        if (saturation < 0.001f)
        {
            return new Color(value * 255f, value * 255f, value * 255f);
        }

        value = Mathf.Clamp(value, 0.001f, 0.999f);

        float h6 = hue * 6f;
        if (h6 == 6f)
        {
            h6 = 0f;
        }

        int ihue = (int) (h6);
        float p = value * (1f - saturation);
        float q = value * (1f - (saturation * (h6 - (float) ihue)));
        float t = value * (1f - (saturation * (1f - (h6 - (float) ihue))));
        switch (ihue)
        {
            case 0:
                return new Color(value, t, p, alpha);
            case 1:
                return new Color(q, value, p, alpha);
            case 2:
                return new Color(p, value, t, alpha);
            case 3:
                return new Color(p, q, value, alpha);
            case 4:
                return new Color(t, p, value, alpha);
            default:
                return new Color(value, p, q, alpha);
        }
    }

    #endregion
}